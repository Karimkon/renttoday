...
<form action="{{ route('secretary.apartments.update', $apartment) }}" method="POST">
    @csrf
    @method('PUT')
    ...
    <div class="mb-3">
        ...
        <select name="tenant_id" class="form-select">
            ...
            @foreach($tenants as $tenant)
                <option value="{{ $tenant->id }}" {{ (string) old('tenant_id', $apartment->tenant_id) === (string) $tenant->id ? 'selected' : '' }}>
                    {{ $tenant->name }}
                </option>
            @endforeach
        </select>
